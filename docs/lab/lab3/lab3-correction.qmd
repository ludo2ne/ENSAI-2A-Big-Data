...
- [ ] Download this [Jupyter Notebook](lab3.ipynb){target="_blank"}
...
## The basics of Spark's Structured Streaming 

### The different sources for stream processing in Spark 
...
- [Apache Kafka](https://kafka.apache.org/){.external target="_blank"} an open-source distributed event streaming platform (not show in this lab)
...
![](img/stream_pipeline_twitter_drawio.png)
...
- [ ] Create a stream to this s3 bucket : 
  - `s3a://remipepin/diffusion/ensai/stream_tweet` for SSPCloud
  - `s3://spark-lab-input-data-ensai20222023/stream_tweet/` for AWS
...
### How to output a stream : summary 
...
for _ in range(10): # we use an _ because the variable isn't use. You can use i if you prefere
    sleep(3)
    spark.sql("""
    SELECT * FROM groupedby_lang""").show()
...
for _ in range(10):
    spark.sql("""
    SELECT COUNT(1) FROM count_filtered_tweet""").show()
    sleep(3)
...
for _ in range(10):
    spark.sql("""
    SELECT *FROM count_filtered_tweet""").show()
    sleep(3)
...
```{python}
# Code Here
ukraine_related = tweet_stream\
.withColumn("covid_related", expr("LOWER(data.text) like '%ukraine%'"))\
.select("data.text", "data.lang", "data.public_metrics", "data.created_at")

covid_related_query = covid_related\
.writeStream\
...
for _ in range(10):
    spark.sql("""
    SELECT * FROM ukraine_related_query """).show()
    sleep(3)
...
count_hashtags_query = count_hashtags.writeStream\
...
for _ in range(10):
    spark.sql("""
    SELECT COUNT(DISTINCT(hashtag)) FROM count_hashtag""").show()
    sleep(3)
...
hashtags_metrics_query = hashtags_metrics.writeStream\
...
for _ in range(10):
    spark.sql("""
    SELECT * FROM hashtags_metrics ORDER by count DESC""").show()
    sleep(3)
...
for _ in range(10):
    spark.sql("""
    SELECT * FROM adhesion_metric """).show()
    sleep(3)
...
![](img/lata_data_handling_without_watermarks.png)
...
# word count + classic time window
sliding_windows_tweet_count = tweet_stream.groupBy(
    window(tweet_stream.data.created_at, "10 seconds")).count()

sliding_windows_tweet_count_query = sliding_windows_tweet_count.writeStream\
...
for _ in range(10):
    spark.sql("""
    SELECT * FROM sliding_windows_tweet_count """).show()
    sleep(3)
...
.withColumn("verified", explode("includes.users.verified"))\
.groupBy(window(tweet_stream.data.created_at, "10 seconds"),"verified").count()
...
for _ in range(10):
    spark.sql("""
    SELECT * FROM sliding_windows_tweet_count ORDER BY window """).show(truncate=False)
    sleep(3)
...
# Code Here
sliding_windows_tweet_count = tweet_stream.groupBy(
    window(tweet_stream.data.created_at, "10 seconds", "5 seconds" )).count()
...
![](img/lata_data_handling_with_watermarks.png)
...
.groupBy(window("timestamp", "10 seconds")).count()
...
.queryName("sliding_windows_tweet_count")\
.format('json')\
.option("checkpointLocation", "s3a://rp-lab3-ensai-20202021/history") \
.option("path", "s3a://rp-lab3-ensai-20202021/")\
.start()
...
- [Spark official documentation](https://spark.apache.org/docs/latest/structured-streaming-programming-guide.html#input-sources){target="_blank"}
